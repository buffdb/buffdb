...
async fn demonstrate_transactions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Begin transaction
    let begin_resp = client.begin_transaction(BeginTransactionRequest {
    ...
    // Commit transaction
    let commit_resp = client.commit_transaction(CommitTransactionRequest {
...
async fn demonstrate_fts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(fts_index) = fts_manager.get_index("articles") {
        ...
        let articles = vec![
            ("doc1", "The quick brown fox jumps over the lazy dog. This is a test of full-text search."),
            ("doc2", "Brown bears are found in many parts of North America and Eurasia."),
            ("doc3", "The lazy programmer automated their daily tasks using Rust."),
            ("doc4", "Quick tips for learning Rust: practice daily and read the documentation."),
        ];
...
async fn demonstrate_json_store() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let json_store = JsonStore::<Sqlite>::at_location(
        Location::InMemory,
        "users".to_string(),
    )?;
    ...
    json_store.patch("user:1", "$.profile.city", json!("San Francisco")).await?;
    println!("\n   Updated $.profile.city to 'San Francisco'");
    
    // Add new field
    json_store.patch("user:1", "$.profile.company", json!("BuffDB Inc")).await?;
...
async fn demonstrate_mvcc() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Tx2: READ counter = {:?} (uncommitted changes not visible)", val);
    ...
    println!("   Tx3: READ counter = {:?} (sees committed value)", 
...
async fn integrated_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    json_store.create_fts_index(
        "content_fts".to_string(),
        "$.content".to_string(),
    )?;
    ...
    for (i, article) in articles.iter().enumerate() {
        json_store.insert(format!("article:{}", i), article.clone(), None).await?;
    }
