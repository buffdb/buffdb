...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(async move {
        ...
        kv_store.create_index(IndexConfig {
    ...
    // Begin a transaction
    let begin_response = client.begin_transaction(BeginTransactionRequest {
    ...
    let set_stream = tokio_stream::iter(vec![
        SetRequest {
    ...
    let commit_response = client.commit_transaction(CommitTransactionRequest {
    ...
    if commit_response.success {
        ...
        println!("   Transaction commit failed: {:?}", commit_response.error_message);
    }
    ...
    let more_users = vec![
        ("user:3", r#"{"name": "Charlie", "email": "charlie@example.com", "age": 30}"#),
        ("user:4", r#"{"name": "Diana", "email": "diana@example.com", "age": 25}"#),
        ("user:5", r#"{"name": "Eve", "email": "eve@example.com", "age": 35}"#),
    ];
    ...
    let rollback_response = client.rollback_transaction(
        buffdb::proto::kv::RollbackTransactionRequest {
            transaction_id: transaction_id.clone(),
        }
    ).await?.into_inner();
    ...
    client.commit_transaction(CommitTransactionRequest {
        transaction_id,
    }).await?;
...
fn demonstrate_index_usage() {
    ...
    index_manager.update_indexes("user:1", None, "alice@example.com").unwrap();
    ...
    if let Some(age_idx) = index_manager.get_index("age_idx") {
        let keys = age_idx.find_range(
            &IndexValue::Integer(25),
            &IndexValue::Integer(35),
        ).unwrap();
