...
impl Default for Tokenizer {
    fn default() -> Self {
        ...
        for word in &["a", "an", "and", "are", "as", "at", "be", "by", "for", 
...
impl FtsIndex {
    ...
    pub fn search(&self, query: &str, limit: usize) -> Vec<SearchResult> {
            ...
            if let Some(docs) = inverted_index.get(term) {
                ...
                for doc_info in docs {
                    ...
                    let score = idf * (tf * (k1 + 1.0)) / 
                               (tf + k1 * (1.0 - b + b * normalized_len));
                    
                    *scores.entry(doc_info.doc_id.clone()).or_insert(0.0) += score;
                    doc_terms.entry(doc_info.doc_id.clone())
        ...
        let mut results: Vec<_> = scores.into_iter()
            .map(|(doc_id, score)| {
                ...
                SearchResult { doc_id, score, highlights }
            })
            .collect();
    ...
    pub fn phrase_search(&self, phrase: &str, limit: usize) -> Vec<SearchResult> {
        ...
        for (term, _) in &phrase_terms {
            if let Some(docs) = inverted_index.get(term) {
                let doc_ids: HashSet<String> = docs.iter()
...
impl FtsManager {
    ...
    pub fn create_index_with_tokenizer(
        ...
        indexes.insert(name.clone(), FtsIndex::with_tokenizer(name.clone(), tokenizer));
        Ok(())
    }
...
mod tests {
    ...
    fn test_basic_search() {
        ...
        index.index_document("doc1".to_string(), "The quick brown fox jumps over the lazy dog").unwrap();
        index.index_document("doc2".to_string(), "The lazy cat sleeps under the warm sun").unwrap();
        index.index_document("doc3".to_string(), "Quick brown rabbits hop through the garden").unwrap();
    ...
    fn test_phrase_search() {
        ...
        index.index_document("doc1".to_string(), "The quick brown fox jumps over the lazy dog").unwrap();
        index.index_document("doc2".to_string(), "The brown quick fox is not here").unwrap();
